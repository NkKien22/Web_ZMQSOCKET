    ...
    <div className="container mt-5">
      <Row>
        ...
        <Col sm={7} className="float-start">
          <div className="ms-5">
            <h4 className="fw-bold float-start w-100">{data?.productName}
            </h4>
          </div>
          <div className="ms-5">
            <h4 className="fw-bold float-start text-danger w-100">{formatPrice(data?.price)}
          </h4>
          </div>
          <div className="ms-5">
            {data?.options?.map(x => (
              <div>{x.optionName}: {x.optionValue}</div>
            ))}
          </div>
